...
| clip_c | `data/benchmark/clips/clip_c_walk.mp4` | 歩行（代用・要実写） | Pexels #5716913。実写は `scripts/capture_benchmark_clip.sh` |

> **clip_c 実写**: WSL 環境にカメラなし（`/dev/video0` 未検出）。スマホ撮影 or USB パススルー後に `capture_benchmark_clip.sh` で差し替え推奨。

## 定量比較（motion-data-2dto3d）

出典: `data/benchmark/reports/summary_20260708T162133Z.json`
